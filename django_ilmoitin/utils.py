...
def send_notification(
    ...
    if not template:
        logger.warning(
            'No notification template created for "{}" event, not sending anything.'.format(  # noqa
                notification_type
            )
        )
        return
    ...
    if not subject:
        logger.warning(
            'Rendered notification "{}" has an empty subject, not sending anything.'.format(  # noqa
                notification_type
            )
        )
        return
...
def render_preview(
    ...
    """
    Return a rendered preview of the passed template and the values
    from the dummy context
