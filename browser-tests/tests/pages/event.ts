...
export const fillFormAdd = async (t: TestController, projectName: string, eventGroupName: string) => {
  const eventGroupOption = eventAdd.eventGroup.find('option');
  const eventGroupRegexp = new RegExp(eventGroupName, "i");

  await t
    .click(eventAdd.project).click(getDropdownOption(projectName))
...
// create new event
export const createNew = async (t: TestController, projectName: string, eventGroupName: string) => {
  await t.navigateTo(routeAdd());

  await fillFormAdd(t, projectName, eventGroupName);

  await t
    .click(eventAdd.saveButton);
};