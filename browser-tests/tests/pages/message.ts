...
export const fillFormAdd = async (t: TestController, projectName: string, eventName: string) => {
  const eventOption = messageAdd.event.find('option');
  const eventRegexp = new RegExp(eventName, "i");

  await t
    .click(messageAdd.project).click(getDropdownOption(projectName))
...
// create new message
export const createNewMessage = async (t: TestController, projectName: string, eventName: string) => {
  await t.navigateTo(routeAdd());

  await fillFormAdd(t, projectName, eventName);

  await t
    .click(messageAdd.saveButton);
};
