...
class BrowserTestAwareJWTAuthentication(RequestJWTAuthentication):
    def __init__(self):
        super().__init__()
        self._api_token_auth_settings = ApiTokenAuthSettings(
            **{
        ...
        if self._api_token_auth_settings.ENABLED:
            ...
            if not is_valid_256_bit_key(
                self._api_token_auth_settings.JWT_SIGN_SECRET
            ):  # Each byte is 8 bits
                raise ImproperlyConfigured(
    ...
    def has_auth_token_for_testing(self, request) -> Optional[JWT]:
        ...
        issued for end-to-ebd browser testing use only.
        ...
        if jwt.claims.get("iss") != self._api_token_auth_settings.ISSUER:
            return None
        return jwt
